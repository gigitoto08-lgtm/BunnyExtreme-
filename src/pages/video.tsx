...
const VideoPage = () => {
  ...
  useEffect(() => {
    if (!video) return;
    const related = videos.filter(v => v.id !== video.id).filter(v =>
      v.tags.some(tag => video.tags.includes(tag))
    ).slice(0,8);
    setTimeout(() => { setRelatedVideos(related); setRelatedLoading(false); }, 500);
  }, [video]);

  if (!video) return <div className="text-center py-20 text-white">Video not found</div>;

  return (
    <div className="min-h-screen bg-black text-white">
      <Navbar />
      <div className="max-w-6xl mx-auto px-4 py-6">
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 mb-6 text-sm text-gray-400 hover:text-primary transition">
          <ArrowLeft className="w-4 h-4"/> Back
        </button>
        <div className="relative w-full aspect-video bg-black rounded-lg overflow-hidden mb-6 shadow-lg">
          {loading && <div className="absolute inset-0 flex items-center justify-center bg-black z-10">
            <div className="w-12 h-12 border-4 border-primary border-t-transparent rounded-full animate-spin"></div>
          </div>}
          <div className={`w-full h-full ${loading ? "opacity-0" : "opacity-100 transition-opacity duration-500"}`} dangerouslySetInnerHTML={{__html: video.embedCode}} />
        </div>
        <h1 className="text-2xl md:text-3xl font-bold mb-3 text-primary">{video.title}</h1>
        <div className="flex items-center gap-4 text-sm text-gray-400 mb-4">
          <span className="flex items-center gap-1"><Eye className="w-4 h-4"/>{formatViews(views)}</span>
        ...
        {relatedLoading ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Array.from({length:8}).map((_,i) => <div key={i} className="bg-gray-900 rounded-lg animate-pulse h-40"></div>)}
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {relatedVideos.map(v => (
              <a key={v.id} href={`/video/${v.id}`} className="block bg-gray-900 rounded overflow-hidden hover:scale-105 hover:shadow-neon transition transform duration-300">
